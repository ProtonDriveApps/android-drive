...
class ConfirmStopAllSharingDialogViewModel @Inject constructor(
    ...
    private val shareId = ShareId(userId, savedStateHandle.require(SHARE_ID))

    val initialViewState = ConfirmStopSharingViewState()
    private val isLoading =
        MutableSharedFlow<Boolean>(replay = 1).apply { tryEmit(initialViewState.isLoading) }
    private val errorMessage =
        MutableSharedFlow<String?>().apply { tryEmit(initialViewState.errorMessage) }
    ...
    private suspend fun deleteShare(confirm: () -> Unit) {
        isLoading.emit(true)
        errorMessage.emit(null)
        val result = deleteShare(shareId, force = true)
        isLoading.emit(false)
        result.onSuccess {
            broadcastMessages(
                userId = userId,
                message = context.getString(I18N.string.description_files_stop_all_sharing_action_success),
                type = BroadcastMessage.Type.SUCCESS,
            )
            confirm()
        }.onFailure { error ->
            errorMessage.emit(context.getString(I18N.string.description_files_stop_all_sharing_action_error))
