...
object PickerPhotosTabRobot : PickerPhotosRobot, LinksRobot, NavigationBarRobot {

    private val photosContent get() = node.withTag(PhotosTestTag.content)
    private val screen get() = node.withTag(PickerPhotosScreenTestTag.screen)

    fun scrollToEnd() = PickerPhotosTabRobot.apply {
        photosContent.swipeUp()
    }

    fun clickOnPhoto(name: String) = photoWithName(name).clickTo(PickerPhotosTabRobot)
