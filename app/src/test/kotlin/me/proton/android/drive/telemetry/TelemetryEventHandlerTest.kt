...
import me.proton.core.drive.announce.event.domain.entity.Event.Upload.UploadState.UPLOAD_FAILED
...
class TelemetryEventHandlerTest {
    ...
    fun backupEnabled() = runTest {
        handler.onEvent(userId, Event.BackupEnabled)

        assertEquals(
            events(PhotosEvent.SettingEnabled()),
            manager.events,
        )
    }
    ...
    fun backupDisabled() = runTest {
        handler.onEvent(userId, Event.BackupDisabled)

        assertEquals(
            events(PhotosEvent.SettingDisabled()),
            manager.events,
        )
    }

    @Test
    fun uploadDone_COMPLETE() = runTest {
        ...
        assertEquals(
            events(PhotosEvent.UploadDone(10, 123, PhotosEvent.Reason.COMPLETED)),
            manager.events,
        )
    }

    @Test
    fun uploadDone_FAILED() = runTest {
        val id = database.db.linkUploadDao.insert(
            LinkUploadEntity(
                userId = me.proton.core.drive.db.test.userId,
                volumeId = volumeId,
        ...
        assertEquals(
            events(PhotosEvent.UploadDone(10, 123, PhotosEvent.Reason.FAILED_OTHER)),
            manager.events,
        )
    }

    @Test
    fun backupState_COMPLETE() = runTest {
        updateUploadStats(
            UploadStats(
                folderId = folderId,
                count = 10,
        ...
        assertEquals(
            events(
                PhotosEvent.BackupStopped(
                    ...
                    files = 10,
                    size = 123,
                    reason = PhotosEvent.Reason.COMPLETED,
        ...
        assertEquals(
            events(
                PhotosEvent.BackupStopped(
                    ...
                    files = 10,
                    size = 123,
                    reason = PhotosEvent.Reason.FAILED_OTHER,
                    isInitialBackup = true,
                )
            ),
            manager.events,
    ...
    @Test
    fun backupStopped_PAUSED_DISABLED_no_stats() = runTest {
        handler.onEvent(
            userId, Event.BackupStopped(
        ...
        assertEquals(
            events(
                PhotosEvent.BackupStopped(
                    ...
                    files = 0,
                    size = 0,
                    reason = PhotosEvent.Reason.PAUSED_DISABLED,
                    isInitialBackup = true,
                )
            ),
            manager.events,
    ...
    private fun events(vararg events: DriveTelemetryEvent) =
        mapOf(userId to events.map { it.zeroTimestamp() })
}
