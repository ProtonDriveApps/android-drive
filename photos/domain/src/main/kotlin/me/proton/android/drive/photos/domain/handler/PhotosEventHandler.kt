...
class PhotosEventHandler @Inject constructor(
    private val photoSyncLinkFolder: PhotoSyncLinkFolder,
) : EventHandler {
    override suspend fun onEvent(userId: UserId, event: Event) {
        if (event is Event.BackupCompleted) {
            photoSyncLinkFolder(userId)
        }
    }
}
