...
class EnablePhotosBackupImpl @Inject constructor(
    ...
    override suspend operator fun invoke(userId: UserId): Result<PhotoBackupState> = coRunCatching {
        when (permissionsManager.getBackupPermissions(refresh = true)) {
            BackupPermissions.Granted -> enableBackup(userId)
    ...
    private suspend fun enableBackup(
        userId: UserId,
    ): PhotoBackupState = if (backupManager.isEnabled(userId).first().not()) {
        val folderName = configurationProvider.backupDefaultBucketName
        setupPhotosBackup(userId, folderName).getOrThrow().let { backupFolders ->
            ...
            if (state is PhotoBackupState.Enabled) {
                announceEvent(userId, Event.BackupEnabled)
            }
            startBackup(userId)
        }
    } else {
