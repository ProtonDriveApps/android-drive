...
class EncryptSessionKey @Inject constructor(
    ...
    ): Result<KeyPacket> = unlockKey(encryptKey, coroutineContext) { key ->
        invoke(key, sessionKey, coroutineContext).getOrThrow()
    }
    ...
    suspend operator fun invoke(
        ...
        sessionKey: SessionKey,
        coroutineContext: CoroutineContext = CryptoScope.EncryptAndDecrypt.coroutineContext,
    ): Result<KeyPacket> = coRunCatching(coroutineContext) {
        withUnlockedKey(unlockedKey) { key ->
            key.encryptSessionKey(sessionKey)
        }
    }
    ...
    suspend operator fun invoke(
        ...
        cryptoContext.pgpCrypto.encryptSessionKeyWithPassword(sessionKey, password)
    }
}
