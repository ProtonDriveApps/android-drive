...
class DecryptFiles @Inject constructor(
    ...
    ): Result<List<File>> = coRunCatching(coroutineContext) {
        decryptKey.useKeysAs(cryptoContext) { keys ->
            val sessionKey = keys.decryptSessionKey(keyPacket)
            input.mapIndexed { index, file ->
                decryptFile(sessionKey, file, output[index], coroutineContext).getOrThrow()
            }
        }
    }
}
