...
data object LastModifiedSorter : Sorter() {
    ...
    override fun sort(driveLinks: List<DriveLink>, direction: Direction): List<DriveLink> =
        driveLinks.sortedWith(
            compareBy<DriveLink> { driveLink -> if (driveLink is DriveLink.Folder) 0 else 1 }
                ...
                .run {
                    when (direction) {
                        Direction.ASCENDING -> thenBy { driveLink -> driveLink.lastModified.value }
                        Direction.DESCENDING -> thenByDescending { driveLink -> driveLink.lastModified.value }
                    }
                }
                .run {
                    when (direction) {
