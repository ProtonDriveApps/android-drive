...
class LogRepositoryImpl @Inject constructor(
    ...
    override suspend fun insert(log: Log) = db.inTransaction {
        db.logDao.insertOrIgnore(log.toLogEntity())
        db.logDao.dropOldRowsToFitLimit(userId = log.userId, limit = configurationProvider.logDbLimit)
    }

    override suspend fun getAllLogs(userId: UserId): List<Log> = db.inTransaction {
        pagedList(
            pageSize = configurationProvider.dbPageSize,
            page = { fromIndex, count ->
                db.logDao.getLogs(userId, count, fromIndex)
            }
        ).map { logEntity -> logEntity.toLog() }
    }
