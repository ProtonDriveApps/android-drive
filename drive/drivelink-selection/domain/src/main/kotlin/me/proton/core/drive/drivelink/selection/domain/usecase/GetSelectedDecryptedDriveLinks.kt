...
class GetSelectedDecryptedDriveLinks @Inject constructor(
    private val getSelectedDriveLinks: GetSelectedDriveLinks,
    private val decryptDriveLinks: DecryptDriveLinks,
) {
    suspend operator fun invoke(selectionId: SelectionId) =
        getSelectedDriveLinks(selectionId).map { list ->
            decryptDriveLinks(list)
        }
}
