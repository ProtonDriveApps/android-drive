...
class GenerateNodeKey @Inject constructor(
    private val generateNestedPrivateKey: GenerateNestedPrivateKey,
) {
    suspend operator fun invoke(userId: UserId, parent: Key, signatureAddress: String): Result<Key.Node> = coRunCatching {
        NodeKey(
            key = generateNestedPrivateKey(userId, parent.keyHolder, signatureAddress).getOrThrow(),
            parent = parent,
        )
    }
}
