...
class TagsMigrationUpdateTagsWorkerTest {
    ...
    fun `happy path`() = runTest {
        ...
        val worker = tagsMigrationUpdateTagsWorker(photoVolumeId, fileId)
    ...
    fun `retryable server error`() = runTest {
        driveRule.server.postLinkTags { retryableErrorResponse() }
    ...
    private fun tagsMigrationUpdateTagsWorker(
        volumeId: VolumeId,
        fileId: FileId,
    ): TagsMigrationUpdateTagsWorker {
        val context = ApplicationProvider.getApplicationContext<Context>()
        return TestListenableWorkerBuilder<TagsMigrationUpdateTagsWorker>(context)
            .setWorkerFactory(object : WorkerFactory() {
                override fun createWorker(
                    ...
                    updateTags = tagsMigrationUpdateTags,
                )
            })
            .setInputData(
                TagsMigrationUpdateTagsWorker.workDataOf(volumeId, fileId)
            )
            .build()
    }
