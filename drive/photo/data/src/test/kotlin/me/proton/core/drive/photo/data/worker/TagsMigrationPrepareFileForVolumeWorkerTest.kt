...
import me.proton.core.drive.photo.domain.usecase.TagsMigrationPrepareFileForVolume
...
class TagsMigrationPrepareFileForVolumeWorkerTest {
    ...
    @Inject
    lateinit var prepareFileForVolume: TagsMigrationPrepareFileForVolume
    ...
    fun `happy path`() = runTest {
        val worker = tagsMigrationPrepareFileForVolumeWorker(userId, photoVolumeId)
        val result = worker.doWork()

        assertEquals(ListenableWorker.Result.success(), result)
    }

    private fun tagsMigrationPrepareFileForVolumeWorker(
        userId: UserId,
        volumeId: VolumeId,
    ): TagsMigrationPrepareFileForVolumeWorker {
        val context = ApplicationProvider.getApplicationContext<Context>()
        return TestListenableWorkerBuilder<TagsMigrationPrepareFileForVolumeWorker>(context)
            .setWorkerFactory(object : WorkerFactory() {
                override fun createWorker(
                    ...
                    prepareFileForVolume = prepareFileForVolume,
                )
            })
            .setInputData(
                TagsMigrationPrepareFileForVolumeWorker.workDataOf(userId, volumeId)
            )
            .build()
    }
