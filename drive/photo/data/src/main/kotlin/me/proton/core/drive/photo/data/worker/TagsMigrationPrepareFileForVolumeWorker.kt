...
@HiltWorker
class TagsMigrationPrepareFileForVolumeWorker @AssistedInject constructor(
    @Assisted appContext: Context,
    @Assisted workerParams: WorkerParameters,
    private val prepareFileForVolume: TagsMigrationPrepareFileForVolume,
) : CoroutineWorker(
    ...
    override suspend fun doWork() = prepareFileForVolume(userId, volumeId).fold(
        onSuccess = {
            Result.success()
        },
        onFailure = { error ->
            if (error.isRetryable) {
                ...
                error.log(LogTag.PHOTO, "Failed to prepare file for photos")
                Result.failure()
            }
        }
    )
    ...
    companion object {
        fun getWorkRequest(
            ...
            OneTimeWorkRequest.Builder(TagsMigrationPrepareFileForVolumeWorker::class.java)
                .setConstraints(
                    Constraints.Builder()
