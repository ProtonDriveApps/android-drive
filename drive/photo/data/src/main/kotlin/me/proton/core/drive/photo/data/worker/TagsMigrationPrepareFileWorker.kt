...
@HiltWorker
class TagsMigrationPrepareFileWorker @AssistedInject constructor(
    ...
    override suspend fun doWork() = prepareFile(
        volumeId = volumeId,
        fileId = fileId,
        startTagging = true,
    ).fold(
        ...
        onFailure = { error ->
            if (error.isRetryable) {
                ...
                error.log(LogTag.PHOTO, "Failed to prepare file for photo ${fileId.id.logId()}")
                Result.failure()
            }
        }
    )
    ...
    companion object {
        fun getWorkRequest(
            ...
            OneTimeWorkRequest.Builder(TagsMigrationPrepareFileWorker::class.java)
                .setConstraints(
                    Constraints.Builder()
