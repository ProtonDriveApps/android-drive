...
@HiltWorker
class TagsMigrationUpdateTagsForVolumeWorker @AssistedInject constructor(
    @Assisted appContext: Context,
    @Assisted workerParams: WorkerParameters,
    private val updateTagsForVolume: TagsMigrationUpdateTagsForVolume,
) : CoroutineWorker(
    ...
    override suspend fun doWork() = updateTagsForVolume(userId, volumeId).fold(
        onSuccess = {
            Result.success()
        },
        onFailure = { error ->
            if (error.isRetryable) {
                ...
                error.log(LogTag.PHOTO, "Cannot update tags from photos")
                Result.failure()
            }
        }
    )
    ...
    companion object {
        fun getWorkRequest(
            ...
            OneTimeWorkRequest.Builder(TagsMigrationUpdateTagsForVolumeWorker::class.java)
                .setConstraints(
                    Constraints.Builder()
