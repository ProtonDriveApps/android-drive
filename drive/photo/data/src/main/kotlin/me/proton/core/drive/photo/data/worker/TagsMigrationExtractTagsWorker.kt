...
@HiltWorker
class TagsMigrationExtractTagsWorker @AssistedInject constructor(
    @Assisted appContext: Context,
    @Assisted workerParams: WorkerParameters,
    private val extractTags: TagsMigrationExtractTags,
) : CoroutineWorker(
    ...
    override suspend fun doWork() = extractTags(volumeId, fileId).fold(
        onSuccess = {
            Result.success()
        },
        onFailure = { error ->
            if (error.isRetryable) {
                ...
                error.log(LogTag.PHOTO, "Cannot extract tags from photo ${fileId.id.logId()}")
                Result.failure()
            }
        }
    )
    ...
    companion object {
        fun getWorkRequest(
            ...
            OneTimeWorkRequest.Builder(TagsMigrationExtractTagsWorker::class.java)
                .setConstraints(
                    Constraints.Builder()
