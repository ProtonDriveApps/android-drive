...
class GetTagsMigrationDownloadedFile @Inject constructor(
    private val repository: TagsMigrationRepository,
) {
    suspend operator fun invoke(userId: UserId, volumeId: VolumeId): Flow<TagsMigrationFile?> =
        repository.getLatestDownloadedFile(userId, volumeId)
}
