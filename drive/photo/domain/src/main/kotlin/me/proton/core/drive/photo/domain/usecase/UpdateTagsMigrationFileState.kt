...
class UpdateTagsMigrationFileState @Inject constructor(
    private val repository: TagsMigrationRepository,
) {
    suspend operator fun invoke(volumeId: VolumeId, fileId: FileId, state: TagsMigrationFile.State) = coRunCatching {
        repository.updateState(volumeId, fileId, state)
    }
}
