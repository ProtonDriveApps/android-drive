...
class TagsMigrationExtractTags @Inject constructor(
    private val updateTagsMigrationFileState: UpdateTagsMigrationFileState,
    private val extractTags: ExtractTags,
    private val insertTagsMigrationFileTags: InsertTagsMigrationFileTags,
    ...
    suspend operator fun invoke(volumeId: VolumeId, fileId: FileId) = coRunCatching {
        val migrationFile = getTagsMigrationFile(volumeId, fileId).getOrThrow()
        val tags = extractTags(fileId, migrationFile?.uriString).getOrThrow()
        insertTagsMigrationFileTags(volumeId, fileId, tags).getOrThrow()
        updateTagsMigrationFileState(volumeId, fileId, EXTRACTED).getOrThrow()
