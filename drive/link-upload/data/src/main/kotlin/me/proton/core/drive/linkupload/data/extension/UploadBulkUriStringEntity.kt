...
fun UploadBulkUriStringEntity.toUploadFileDescription() = UploadFileDescription(
    uri = uri,
    properties = if (name != null && mimeType != null && size != null && lastModified != null) {
        UploadFileProperties(
            name = name,
            mimeType = mimeType,
            size = Bytes(size),
            lastModified = TimestampMs(lastModified)
        )
    } else {
        null
    }
