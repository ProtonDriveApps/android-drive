...
@Singleton
class WithDriveLinkFile @Inject constructor(
    val withDriveLink: WithDriveLink,
) {

    suspend inline operator fun <T> invoke(
        documentId: DocumentId,
        crossinline block: suspend (UserId, DriveLink.File) -> T,
    ) = withDriveLink(documentId) { userId, driveLink ->
        require(driveLink is DriveLink.File) { "documentId must point to a FileId" }
