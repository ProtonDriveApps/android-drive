...
class CancelUserMessage @Inject constructor(
    private val repository: UserMessageRepository,
) {
    suspend operator fun invoke(userId: UserId, message: UserMessage) = coRunCatching {
        repository.insertOrUpdate(userId, message)
    }
}
