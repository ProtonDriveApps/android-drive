...
import me.proton.core.drive.user.data.db.dao.UserMessageDao

interface UserMessageDatabase : Database {
    val quotaDao: QuotaDao
    val userMessageDao: UserMessageDao
    ...
    companion object {
        ...
        val MIGRATION_1 = object : DatabaseMigration {
            override fun migrate(database: SupportSQLiteDatabase) {
                database.execSQL(
                    """
                    CREATE TABLE IF NOT EXISTS `DismissedUserMessageEntity` (
                    `user_id` TEXT NOT NULL,
                    `user_message` TEXT NOT NULL,
                    `update_time` INTEGER NOT NULL,
                    PRIMARY KEY(`user_id`, `user_message`),
                    FOREIGN KEY(`user_id`) REFERENCES `AccountEntity`(`userId`)
                    ON UPDATE NO ACTION ON DELETE CASCADE )
                    """.trimIndent()
                )
