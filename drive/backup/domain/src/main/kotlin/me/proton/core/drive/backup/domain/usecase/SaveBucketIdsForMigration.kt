...
class SaveBucketIdsForMigration @Inject constructor(
    private val getAllFolders: GetAllFolders,
    private val migrationRepository: PhotoShareMigrationRepository,
) {
    ...
    suspend operator fun invoke(folderId: FolderId) = coRunCatching {
        getAllFolders(folderId).getOrThrow()
            ...
            ?.let { backupFolders ->
                migrationRepository.saveBucketIds(
                    userId = folderId.userId,
                    bucketsIds = backupFolders.map { backupFolder -> backupFolder.bucketId },
                )
            }
    }
}
