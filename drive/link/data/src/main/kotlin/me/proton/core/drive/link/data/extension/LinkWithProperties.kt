...
fun LinkWithProperties.toLink(): Link {
    val shareId = ShareId(link.userId, link.shareId)
    return when (properties) {
        is LinkFilePropertiesEntity -> Link.File(
            id = FileId(shareId, link.id),
            parentId = parentId,
            name = link.name,
            ...
            tags = tags.mapNotNull { value -> PhotoTag.fromLong(value) },
        ...
        is LinkAlbumPropertiesEntity -> Link.Album(
            ...
            coverLinkId = properties.coverLinkId?.let { FileId(shareId, it) },
        )
    }
}
