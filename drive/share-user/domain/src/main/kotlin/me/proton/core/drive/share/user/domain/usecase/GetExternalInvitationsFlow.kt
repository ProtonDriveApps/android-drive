...
class GetExternalInvitationsFlow @Inject constructor(
    private val repository: ShareExternalInvitationRepository,
    private val configurationProvider: ConfigurationProvider,
) {
    operator fun invoke(
        shareId: ShareId,
        refresh: Flow<Boolean> = flowOf { !repository.hasExternalInvitations(shareId) }
    ): Flow<DataResult<List<ShareUser.ExternalInvitee>>> = refresh.transform { shouldRefresh ->
        if (shouldRefresh) {
            fetcher {
                repository.fetchExternalInvitations(shareId)
            }
        }
        emitAll(repository.getExternalInvitationsFlow(
            shareId = shareId,
            limit = configurationProvider.dbPageSize,
        ).map { invitees -> invitees.asSuccess })
    }
}
