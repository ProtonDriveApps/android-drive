...
class GetInvitationsFlow @Inject constructor(
    private val repository: ShareInvitationRepository,
    private val configurationProvider: ConfigurationProvider,
) {
    operator fun invoke(
        shareId: ShareId,
        refresh: Flow<Boolean> = flowOf { !repository.hasInvitations(shareId) }
    ): Flow<DataResult<List<ShareUser.Invitee>>> = refresh.transform { shouldRefresh ->
        if (shouldRefresh) {
            fetcher {
                repository.fetchInvitations(shareId)
            }
        }
        emitAll(repository.getInvitationsFlow(
            shareId = shareId,
