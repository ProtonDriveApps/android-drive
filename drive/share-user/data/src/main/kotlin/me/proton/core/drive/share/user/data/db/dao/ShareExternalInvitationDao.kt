...
@Dao
abstract class ShareExternalInvitationDao : BaseDao<ShareExternalInvitationEntity>() {

    @Query(
        """SELECT EXISTS(
            SELECT * FROM ShareExternalInvitationEntity
    ...
    abstract fun getInvitationsFlow(
        ...
        shareId: String,
        limit: Int,
    ): Flow<List<ShareExternalInvitationEntity>>

    @Query(
        """
            DELETE FROM ShareExternalInvitationEntity
            WHERE user_id = :userId AND share_id = :shareId
        """
    )
    ...
    abstract fun getInvitationFlow(
        ...
        shareId: String,
        invitationId: String,
    ): Flow<ShareExternalInvitationEntity?>

    @Query(
        """
            UPDATE ShareExternalInvitationEntity SET permissions = :permissions
            WHERE user_id = :userId AND share_id = :shareId AND id = :invitationId
        """
    )
